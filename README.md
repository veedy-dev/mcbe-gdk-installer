...
experimental `v0.2.0-experimental` engine instead starts authentication from
Minecraft's own **Sign In** button.
...
displayed Microsoft device-code flow. With `v0.2.0-experimental`, launch
Minecraft and select **Sign In** inside the game. The launcher supervises the
engine's `login.json` request and presents the Microsoft URL and code without
moving the actual authentication back into the launcher or CLI.
...
### Experimental engine: v0.2.0-experimental

The setup UI exposes **MCBE GDK v0.2.0-experimental**. This is a source-built
prerelease from the maintained public WineGDK tree and the same pinned
GDK-Proton, graphics, Debian, and Steam Runtime inputs as stable releases. It
fixes the Gaming Services version-override reader and runs its focused
`xgameruntime` regression during the pinned build.

It can also be selected from the terminal using its exact release asset:

```bash
mcbe-gdk-linux engine \
  'https://github.com/veedy-dev/mcbe-gdk-engine/releases/download/v0.2.0-experimental/GDK-Proton-mcbe-gdk-v0.2.0-experimental.tar.gz'
```
...
members, and revalidates critical installed files. The exact experimental
profile transactionally applies the Minecraft title identity, the Windows App
Runtime workaround, and WineGDK's Gaming Services registry override. The
profile activates only when the repository, tag, asset, and SHA-256 all match.
...
**Parties and Realms remain unsupported.** Keep the stable engine available
while comparing launch time, server join time, frame pacing, and server tick
time. The selection persists across normal updates. For unattended initial
installs, set `MCBE_GDK_ENGINE_RELEASE` to the complete asset URL above.
...
| `v0.2.0-experimental` sign in/out | Use the Profile controls inside Minecraft |
...
`v0.2.0-experimental` is built from the maintained source with pinned inputs,
release checksums, a manifest, focused Wine tests, and reproducible packaging.
